...
Module: Perception Front-end (V2 - One-shot Depth Prior Batch Evaluation)

Description:
Implements the One-shot Calibration and Zero-shot Generalization paradigm using Depth Anything V2.
...
mIoU: {metrics['miou']:.4f}
  - Foreground IoU: {metrics['fg_iou']:.4f}
  - Background IoU: {metrics['bg_iou']:.4f}

PA (Pixel Accuracy): {metrics['pa']:.4f}
...
"""
    ...
    if metrics['iou'] >= 0.7 and metrics['pa'] >= 0.85: eval_text += "\n- Excellent results, optimal parameters!"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='One-shot Depth Prior Batch Evaluation for Depth Anything V2')
    ...
    parser.add_argument('--outdir', type=str, default='./v2_oneshot_eval_results')
    ...
    print(f"Loading Depth Anything V2 Model ({args.encoder})...")
    ...
    print("Running V2 depth estimation on reference image...")
    ref_depth_raw = depth_anything.infer_image(ref_raw, args.input_size)
    ref_depth_norm = (ref_depth_raw - ref_depth_raw.min()) / (ref_depth_raw.max() - ref_depth_raw.min()) * 255
    ...
    for img_path in img_list:
        ...
        if args.keep_largest:
            ...
            if args.min_area > 0:
                num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(depth_mask, connectivity=8)
        ...
        for idx, polygon_data in enumerate(polygons):
            ...
            result_path = os.path.join(args.outdir, f"{base_name}_v2_result_{label}_{idx}.png")
            ...
            if args.save_all:
                ...
                cv2.imwrite(os.path.join(args.outdir, f"{base_name}_v2_combined_{label}_{idx}.png"), combined)
    ...
    if len(all_metrics) > 0:
        ...
        summary_path = os.path.join(args.outdir, "FINAL_BATCH_SUMMARY_V2.json")
