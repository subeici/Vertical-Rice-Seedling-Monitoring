...
Module: Perception Front-end (V1 - One-shot Depth Prior Batch Evaluation)

Description:
Implements the One-shot Calibration and Zero-shot Generalization paradigm.
Uses a single reference mask to extract depth geometric priors, and applies
this fixed prior to batch process and evaluate the entire dataset.
"""
...
def calculate_iou_and_pa(pred_mask, gt_mask):
    ...
    miou = (bg_iou + iou) / 2
    metrics = {
        'iou': iou, 'miou': miou, 'bg_iou': bg_iou, 'fg_iou': iou,
...
def analyze_depth_distribution(depth_map, polygon_mask):
    ...
    inside_stats = {
        ...
        'percentiles': {1: np.percentile(inside_depth, 1), 5: np.percentile(inside_depth, 5),
    ...
    if len(outside_depth) > 0:
        outside_stats = {
            ...
            'percentiles': {1: np.percentile(outside_depth, 1), 5: np.percentile(outside_depth, 5),
...
def optimize_depth_range_smart(depth_map, polygon_mask, inside_stats, outside_stats,
    ...
    if method == 'grid_search':
        min_candidates = [inside_stats['percentiles'][1], inside_stats['percentiles'][5],
                          inside_stats['percentiles'][10], inside_stats['mean'] - 2 * inside_stats['std'],
                          inside_stats['mean'] - 1.5 * inside_stats['std'], inside_stats['mean'] - inside_stats['std']]
        max_candidates = [inside_stats['mean'] + inside_stats['std'], inside_stats['mean'] + 1.5 * inside_stats['std'],
        ...
        for d_min in min_candidates:
            for d_max in max_candidates:
                if d_min < d_max:
                    score, recall, precision, fpr = calculate_score(d_min, d_max, depth_map, polygon_mask,
                                                                    weight_recall, weight_precision)
                    if score > best_score:
                        best_score = score
                        best_depth_min, best_depth_max = d_min, d_max
        ...
        if outside_stats:
            ...
            best_depth_min, best_depth_max = inside_stats['percentiles'][10], inside_stats['percentiles'][90]
    elif method == 'adaptive':
        std_ratio = inside_stats['std'] / inside_stats['mean'] if inside_stats['mean'] != 0 else 1
        if std_ratio < 0.15:
            best_depth_min, best_depth_max = inside_stats['percentiles'][25], inside_stats['percentiles'][75]
        elif std_ratio < 0.3:
            best_depth_min, best_depth_max = inside_stats['percentiles'][10], inside_stats['percentiles'][90]
        else:
            best_depth_min, best_depth_max = inside_stats['mean'] - inside_stats['std'], inside_stats['mean'] + \
                                             inside_stats['std']
        score, recall, precision, fpr = calculate_score(best_depth_min, best_depth_max, depth_map, polygon_mask,
                                                        weight_recall, weight_precision)
        best_metrics = {'recall': recall, 'precision': precision, 'fpr': fpr, 'score': score}

    return max(0, best_depth_min), min(255, best_depth_max), best_metrics
...
PA (Pixel Accuracy): {metrics['pa']:.2%}
...
Confusion Matrix:
  ...
  TN: {metrics['tn']:,}
"""
    ax8.text(0.05, 0.95, metrics_text, transform=ax8.transAxes, fontsize=10, verticalalignment='top',
    ...
    if metrics['recall'] < 0.6: eval_text += "\n- Low recall, consider relaxing depth range."
    if metrics['precision'] < 0.5: eval_text += "\n- Low precision, consider tightening depth range."
    if metrics['iou'] < 0.5: eval_text += "\n- Low IoU, strategy optimization required."
    if metrics['pa'] < 0.75: eval_text += "\n- Insufficient Pixel Acc, check depth thresholds."
    if metrics['iou'] >= 0.7 and metrics[
        'pa'] >= 0.85: eval_text += "\n- Excellent results, current parameters are optimal!"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='One-shot Depth Prior Batch Evaluation for Depth Anything V1')
    ...
    parser.add_argument('--outdir', type=str, default='./vis_depth_v1_optimized')
    ...
    print(f"Loading Depth Anything V1 Model ({args.encoder})...")
    depth_anything = DepthAnything.from_pretrained('LiheYoung/depth_anything_{}14'.format(args.encoder)).to(
        DEVICE).eval()

    transform = Compose([
        Resize(width=args.input_size, height=args.input_size, resize_target=False, keep_aspect_ratio=True,
    ...
    ref_mask = np.zeros(ref_raw.shape[:2], dtype=np.uint8)
    for p in ref_polygons:
        cv2.fillPoly(ref_mask, [p['points']], 255)

    image = cv2.cvtColor(ref_raw, cv2.COLOR_BGR2RGB) / 255.0
    h, w = image.shape[:2]
    ...
    depth_raw = depth.cpu().numpy()
    ref_depth_norm = (depth_raw - depth_raw.min()) / (depth_raw.max() - depth_raw.min()) * 255.0
    ...
    print(
        f"✅ Calibration Complete! Global Depth Prior Locked: MIN = {GLOBAL_DEPTH_MIN:.2f}, MAX = {GLOBAL_DEPTH_MAX:.2f}")
    ...
    for img_path in img_list:
        ...
        depth = F.interpolate(depth[None], (h, w), mode='bilinear', align_corners=False)[0, 0]
        depth_raw = depth.cpu().numpy()
        depth_normalized = (depth_raw - depth_raw.min()) / (depth_raw.max() - depth_raw.min()) * 255.0

        depth_mask = (depth_normalized >= GLOBAL_DEPTH_MIN) & (depth_normalized <= GLOBAL_DEPTH_MAX)
        ...
        for idx, polygon_data in enumerate(polygons):
            ...
            print(f"Image: {base_name} | Label: {label} | IoU: {iou:.3f} | PA: {pa:.2%}")
            ...
            if args.save_all:
                ...
                cv2.imwrite(os.path.join(args.outdir, f"{base_name}_combined_{label}_{idx}.png"), combined)
    ...
    if len(all_metrics) > 0:
        ...
        print(f"  Global Average IoU:       {avg_iou:.3f}")
        print(f"  Global Average mIoU:      {avg_miou:.3f}")
        print(f"  Global Average PA:        {avg_pa:.2%}")
        print(f"  Global Average Precision: {avg_precision:.2%}")
        print(f"  Global Average Recall:    {avg_recall:.2%}")
        print(f"  Global Average F1 Score:  {avg_f1:.3f}")
        ...
        summary_path = os.path.join(args.outdir, "FINAL_BATCH_SUMMARY.json")
